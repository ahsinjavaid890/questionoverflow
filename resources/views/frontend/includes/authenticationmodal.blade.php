...
<div class="modal fade modal-container signup-form" id="signUpModal" tabindex="-1" role="dialog" aria-labelledby="signUpModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="alert alert-success alert-success-message" style="display:none;background-color: #04AA6D; color: white; max-width: 70rem; margin: auto;">
                                
                </div>
                <div class="alert alert-danger print-error-msg" style="max-width: 70rem; margin: auto;display:none; color: #a94442;background-color: #f2dede;border-color: #ebccd1;">
                    <ul style="text-transform:capitalize;"></ul>
                </div>
                <form method="post" action="{{ url('userregister') }}" id="registerform">
                    @csrf
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <div class="custom-control custom-checkbox fs-14">
                            <input type="checkbox" name="need_to_aggree_our_privacy_policy" class="custom-control-input" id="agreeCheckBox">
                            <label class="custom-control-label custom--control-label" for="agreeCheckBox">By signing up, you agree to our <a href="{{ url('privacy-policy') }}" class="text-color hover-underline">Privacy Policy.</a></label>
                        </div>
                    </div>
                    <div class="btn-box">
                        <button id="submit-button" type="submit" class="btn theme-btn w-100">
                        Register Account <i class="la la-arrow-right icon ml-1"></i>
                    </button>
                    </div>
                    <p class="create-account-text text-right fs-14">
                        Already on Question Overflow? <a class="login-btn text-color hover-underline" href="javascript:void(0)">Log in</a>
                    </p>
...
<script type="text/javascript">
    ...
    $('#submit-button').html('<i style="font-size:22px;" class="la la-spin la-spinner"></i>');
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(data){
         if($.isEmptyObject(data.error)){
            ...
            $('#submit-button').html('Register Account <i class="la la-arrow-right icon ml-1"></i>');
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form method="POST" action="{{ url('customerlogin') }}" id="loginform">
                    @csrf
                    <div class="form-group">
                        <label class="fs-14 text-black fw-medium lh-18">Email</label>
                        <input class="form-control form--control" type="email" name="email" placeholder="Email address">
                    ...
                    <p class="create-account-text text-right fs-14 pt-1">
                        New to Question Overflow? <a class="signup-btn text-color hover-underline" href="javascript:void(0)">Create account</a>
                    </p>
                    <div class="icon-element my-4 mx-auto shadow-sm fs-25">Or</div>
                    <div class="text-center">
...
<script type="text/javascript">
    ...
    $('#submit-button-login').html('<i style="font-size:22px;" class="la la-spin la-spinner"></i>');
    e.preventDefault();
    var formData = new FormData(this);
    $.ajax({
        type:'POST',
